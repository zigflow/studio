...
// Workflow sequencing
//
// Always emits the new "workflows-list" format: every workflow is wrapped
// in its name as a top-level do entry.
// ---------------------------------------------------------------------------
...
function exportRaiseConfig(c: RaiseConfig): Record<string, unknown> {
  ...
  if (c.definition.title) definition['title'] = c.definition.title;
  if (c.definition.detail) definition['detail'] = c.definition.detail;
...
function exportListenConfig(c: ListenConfig): Record<string, unknown> {
  ...
  if (c.mode === 'any') {
    return { listen: { to: { any: events } } };
  }
  return { listen: { to: { all: events } } };
}

// ---------------------------------------------------------------------------
// SwitchNode — branch handling
//
// If a branch carries `thenWorkflowName`, it references a top-level named
// workflow that is already in `file.workflows`; emit `then: <name>` and skip
// hoisting (the workflow is emitted separately by exportWorkflows).
//
// If a branch has no `thenWorkflowName`, generate a name from the node and
// branch labels, hoist the branch graph as a named top-level workflow entry,
// and emit `then: <generatedName>`.
// ---------------------------------------------------------------------------

function exportSwitchNode(node: SwitchNode, ctx: ExportContext): unknown {
  const branches = node.branches.map((branch) => {
    let thenName: string;
    if (branch.thenWorkflowName) {
      // Named workflow reference — already emitted at top level.
      thenName = branch.thenWorkflowName;
    } else {
      // No reference: hoist the branch graph as a new top-level workflow.
      thenName = `${node.name}-${branch.label}`;
      const tasks = exportFlowGraphTasks(branch.graph, ctx);
      ctx.hoisted.push({ name: thenName, tasks });
    }

    const entry: Record<string, unknown> = { then: thenName };
